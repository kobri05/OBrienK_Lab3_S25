...
```{r}
#| label: setup
library(tidyverse) #Loaded in tidyverse to begin analysis
```
...
surveys <- read_csv("data-raw/surveys.csv") #Load data into R and assign object
glimpse(surveys) #Get an overview of data
```

**1. What are the dimensions (# of rows and columns) of these data?**
There are 15 columns and 30,463 rows.
...
The data types are 'character,' 'double,' and 'dates.' 
<!-- Run the commented code in the load-data chunk to address this question!  -->
...
```{r}
#| label: add transparency to scatterplot
ggplot(data = surveys,
       mapping = aes(x = weight,
                     y = hindfoot_length,
                     alpha = 0.3)) +
  geom_point() #'alpha' makes points on scatterplot transparent and in theory easier to read.
```
...
```{r}
#| label: facet scatterplot by species
ggplot(data = surveys,
       mapping = aes(x = weight,
                     y = hindfoot_length,
                     alpha = 0.3)) +
...
```{r}
#| label: add axis labels to scatterplot
ggplot(data = surveys,
       mapping = aes(x = weight,
                     y = hindfoot_length,
                     alpha = 0.3)) +
...
```{r}
#| label: boxplot with individual points
ggplot(data = surveys,
       mapping = aes(y = weight, x = species)) +
  geom_boxplot() +
  geom_point()
...
```{r}
#| label: boxplot with better points
ggplot(data = surveys,
       mapping = aes(y = weight, 
                     x = species)) +
  geom_boxplot() +
  geom_jitter(mapping = aes(color= weight),
              alpha = 0.3,
              color = 'cadetblue')
```
...
```{r}
#| label: boxplot with better points AND more visible
ggplot(data = surveys,
       mapping = aes(y = weight, 
                     x = species)) +
...
ggplot(data = surveys,
  ...
  theme(axis.text.x = element_text(angle = 45)) #used https://www.sthda.com/english/wiki/ggplot2-axis-ticks-a-guide-to-customize-tick-marks-and-labels as a resource to roate text :)
```
